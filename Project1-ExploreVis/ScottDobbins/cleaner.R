...
# recoding levels
WW2_bombs[, Unit_Service     := fct_recode(Unit_Service, 
                                           `RAAF` = "RAAF/NEI")]
WW2_bombs[, Unit_Country     := fct_recode(Unit_Country, 
                                           `UK` = "GREAT BRITAIN")]
WW2_bombs[, Aircraft_Type    := fct_recode(Aircraft_Type, 
                                           `A-31 Vengeance` = "VENGEANCE (A31)", 
                                           `A-31 Vengeance` = "VENGEANCE(A-31)")]
WW2_bombs[, Weapon_Expl_Type := fct_recode(Weapon_Expl_Type, 
                                           TORPEDO = "TORPEDOES", 
                                           TORPEDO = "TORPEDOES MISC", 
                                           `40 LB EXPLOSIVE` = "UNK CODE 20 110 LB EXPLOSIVE", 
                                           `250 LB BAP` = "250 BAP")]
WW2_bombs[, Weapon_Incd_Type := fct_recode(Weapon_Incd_Type, 
                                           `110 LB INCENDIARY` = "110 LB  INCENDIARY")]
WW2_bombs[, Weapon_Frag_Type := fct_recode(Weapon_Frag_Type, 
                                           `138 LB FRAG (6X23 CLUSTERS)` = "23 LB FRAG CLUSTERS (6 X23 PER CLUSTER)", 
                                           `23 LB PARA FRAG` = "23 LB PARAFRAG")]

# dropping levels
WW2_bombs[Target_Type %like% "\\b(UNID|UNDENT)", Target_Type := ""]
...
WW2_bombs[, Sighting_Method_Code := as.integer(Sighting_Method_Code)] # cleaning data type due to bad data
...
if (debug_mode_on) print("cleaning Korea1")

# specific fixes, strings

# recoding levels
Korea_bombs1[, Unit_ID       := fct_recode(Unit_ID, 
                                           GAF = "G AF")]
Korea_bombs1[, Aircraft_Type := fct_recode(Aircraft_Type, 
                                           `C-54` = "O54", 
                                           `L-05` = "LO5", 
                                           `RB-29` = "R829", 
                                           `T-06` = "TO6", 
...
Korea_bombs2[, Aircraft_Type := fct_recode(Aircraft_Type, 
                                           `RB-45` = "RB 45")]
Korea_bombs2[, Mission_Type  := fct_recode(Mission_Type, 
                                           Interdiction = "Interdcition", 
                                           Interdiction = "Interdiciton", 
                                           Interdiction = "Interdicton", 
                                           Interdiction = "Interduction")]
Korea_bombs2[, Nose_Fuze     := fct_recode(Nose_Fuze, 
...
# recoding levels
Vietnam_bombs[, Unit_Country := fct_recode(Unit_Country, 
                                           `South Korea` = "KOREA (SOUTH)", 
                                           `USA` = "UNITED STATES OF AMERICA", 
                                           `South Vietnam` = "VIETNAM (SOUTH)")]
Vietnam_bombs[, Target_Type := fct_recode(Target_Type, 
                                          TROOPS = "TROOP UNK", 
                                          TROOPS = "TROOP UNK #")]
Vietnam_bombs[, Target_Country := fct_recode(Target_Country, 
...
Vietnam_bombs$Mission_Day_Period           %>% setattr("levels", ifelse(levels(.) == "D", "day", 
                                                                 ifelse(levels(.) == "N", "night", 
                                                                 ifelse(levels(.) == "M", "morning", 
                                                                 ifelse(levels(.) == "E", "evening", 
                                                                 "")))))
