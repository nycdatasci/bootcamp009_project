...
server <- function(input, output, session) {#***session is still currently unused below
  ...
  observeEvent(eventExpr = input$pick_map, ignoreNULL = FALSE, handlerExpr = {
    ...
    if(input$pick_map == "Color Map") {
      proxy %>% clearTiles() %>% addProviderTiles("Stamen.Watercolor", layerId = "map_base")#, options = providerTileOptions(attribution = 'Map tiles by <a href="http://stamen.com">Stamen Design</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'))
    } else if(input$pick_map == "Plain Map") {
      proxy %>% clearTiles() %>% addProviderTiles("CartoDB.PositronNoLabels", layerId = "map_base")#, options = providerTileOptions(attribution = '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> &copy; <a href="http://cartodb.com/attributions">CartoDB</a>'))
    } else if(input$pick_map == "Terrain Map") {
      proxy %>% clearTiles() %>% addProviderTiles("Stamen.TerrainBackground", layerId = "map_base")#, options = providerTileOptions(attribution = 'Map tiles by <a href="http://stamen.com">Stamen Design</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'))
    } else if(input$pick_map == "Street Map") {
      proxy %>% clearTiles() %>% addProviderTiles("HERE.basicMap", layerId = "map_base", options = providerTileOptions(app_id = '5LPi1Hu7Aomn8Nv4If6c', app_code = 'mrmfvq4OREjya6Vbjmw6Gw'))#, attribution = 'Map &copy; 2016 <a href="http://developer.here.com">HERE</a>'))
    } else if(input$pick_map == "Satellite Map") {
      proxy %>% clearTiles() %>% addProviderTiles("Esri.WorldImagery", layerId = "map_base")#, options = providerTileOptions(attribution = 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'))
    }
  ...
  observeEvent(eventExpr = input$show_WW1, ignoreNULL = FALSE, ignoreInit = TRUE, handlerExpr = {
    ...
    proxy <- leafletProxy("mymap")
    if(input$show_WW1) {
      proxy %>% addCircles(data = WW1_unique_target, lat = ~Target.Latitude, lng = ~Target.Longitude, color = "darkblue", weight = 5, opacity = 0.5, fill = TRUE, fillColor = "darkblue", fillOpacity = 0.5, popup = ~tooltip, group = "WW1_unique_targets")
    } else {
  ...
  observeEvent(eventExpr = input$show_WW2, ignoreNULL = FALSE, ignoreInit = TRUE, handlerExpr = {
    ...
    proxy <- leafletProxy("mymap")
    if(input$show_WW2) {
      proxy %>% addCircles(data = WW2_unique_target, lat = ~Target.Latitude, lng = ~Target.Longitude, color = "darkred", weight = 5, opacity = 0.5, fill = TRUE, fillColor = "darkred", popup = ~tooltip, group = "WW2_unique_targets")
    } else {
  ...
  observeEvent(eventExpr = input$show_Korea, ignoreNULL = FALSE, ignoreInit = TRUE, handlerExpr = {
    ...
    proxy <- leafletProxy("mymap")
    if(input$show_Korea) {
      proxy %>% addCircles(data = Korea_unique_target, lat = ~Target.Latitude, lng = ~Target.Longitude, color = "yellow", weight = 5, opacity = 0.5, fill = TRUE, fillColor = "yellow", popup = ~tooltip, group = "Korea_unique_targets")
    } else {
  ...
  observeEvent(eventExpr = input$show_Vietnam, ignoreNULL = FALSE, ignoreInit = TRUE, handlerExpr = {
    ...
    if(input$show_Vietnam) {
      proxy %>% addCircles(data = Vietnam_unique_target, lat = ~Target.Latitude, lng = ~Target.Longitude, color = "darkgreen", weight = 5, opacity = 0.5, fill = TRUE, fillColor = "darkgreen", popup = ~tooltip, group = "Vietnam_unique_targets")
    } else {
