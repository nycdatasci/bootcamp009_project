...
class_data$GSPC.Close = cut(class_data$GSPC.Close,c(-10,-2,1,10))
logr = glm(GSPC.Close ~ ., class_data[train.index],family = 'gaussian')
summary(logr)
...
```{r}
model.empty = glm(GSPC.Close ~ 1, data = class_data[train.index]) #The model with an intercept ONLY.
model.full = glm(GSPC.Close ~ ., data = class_data[train.index]) #The model with ALL variables.
...
summary(forwardAIC)
train_pred_forward = predict(forwardAIC,class_data[train.index,1:(ncol(class_data)-1)])
test_pred_forward = predict(forwardAIC,class_data[test.index,1:(ncol(class_data)-1)])
accuracy(as.vector(train_pred_forward),class_data[train.index],(ncol(class_data)))
accuracy(as.vector(test_pred_forward),class_data[test.index,(ncol(class_data))])
summary(backwardAIC)
train_pred_backward = predict(backwardAIC,class_data[train.index,1:(ncol(class_data)-1)])
test_pred_backward = predict(backwardAIC,class_data[test.index,1:(ncol(class_data)-1)])
accuracy(as.vector(train_pred_backward),class_data[train.index],(ncol(class_data)))
accuracy(as.vector(test_pred_backward),class_data[test.index,(ncol(class_data))])

resid_forwlr_test = data.frame(class_data[test.index,ncol(class_data)],test_pred_forward,(class_data[test.index,ncol(class_data)]-test_pred_forward),row.names = index(complete_data[test.index]))
...
```{r}
## Create model and assess accuracy on train/test set.
rf = randomForest(GSPC.Close ~ ., class_data[train.index])
varImpPlot(rf)
rf
train_pred_rf = predict(rf, class_data[train.index,1:(ncol(class_data)-1)])
test_pred_rf = predict(rf, class_data[test.index,1:(ncol(class_data)-1)])
accuracy(as.vector(train_pred_rf),class_data[train.index,ncol(class_data)])
accuracy(as.vector(test_pred_rf), class_data[test.index,ncol(class_data)])

## Collect and visualize residuals from the train and test sets for analysis.
resid_rf = data.frame(rf$y,rf$predicted,(rf$y - rf$predicted),row.names = index(class_data[train.index]))
...
resid_rf_test = data.frame(class_data[test.index,ncol(class_data)],test_pred_rf,(class_data[test.index,ncol(class_data)]-test_pred_rf),row.names = index(class_data[test.index]))
...
```{r}
df = data.frame(class_data[train.index,1:(ncol(class_data)-1)])
