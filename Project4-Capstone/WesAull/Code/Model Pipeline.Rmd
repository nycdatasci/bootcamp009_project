...
## Slightly smooth returns.
GSPC_18M_Ret$GSPC.Close = rollmean(GSPC_18M_Ret$GSPC.Close,2,fill=NA)
GSPC_18M_Ret = GSPC_18M_Ret[complete.cases(GSPC_18M_Ret),]

GSPC_12M_Ret$GSPC.Close = rollmean(GSPC_12M_Ret$GSPC.Close,2,fill=NA)
GSPC_12M_Ret = GSPC_12M_Ret[complete.cases(GSPC_12M_Ret),]

GSPC_6M_Ret$GSPC.Close = rollmean(GSPC_6M_Ret$GSPC.Close,2,fill=NA)
GSPC_6M_Ret = GSPC_6M_Ret[complete.cases(GSPC_6M_Ret),]
...
```{r}
window_length = 60
#exp_decay <- 0.98 ^ (window_length:1)
a = data$LADCB[complete.cases(data$LADCB),]
a = ROC(a$LADCB,6,type='discrete')
data$LADCB = NULL
...
plot(1.2^a)
b=1.2^a
data = merge(data,b,join='left',fill='NA')
```
...
a = rollmeanr(a$NYX_MD,12,fill=NA)
a = a$NYX_MD[complete.cases(a$NYX_MD),]
a = ROC(a$NYX_MD,12,type='discrete')
data$NYX_MD = NULL
a = a$NYX_MD[complete.cases(a$NYX_MD),]
...
window_length = 120
exp_decay <- 0.95 ^ (window_length:1)
a = data$NYX_CB[complete.cases(data$NYX_CB),]
a = rollmeanr(a$NYX_CB,12,fill=NA)
a = a$NYX_CB[complete.cases(a$NYX_CB),]
data$NYX_CB = NULL
a = a$NYX_CB[complete.cases(a$NYX_CB),]
a = roll_scale(a$NYX_CB[complete.cases(a$NYX_CB),],width=window_length,min_obs = 12)
plot(a)
data = merge(data,a,join='left',fill='NA')
```
...
a = data$WKH_MAN[complete.cases(data$WKH_MAN),]
a = rollmeanr(a$WKH_MAN,24,fill=NA)
...
a = rollmeanr(a$RSI_8M,3,fill=NA)
...
window_length = 60
exp_decay <- 0.95 ^ (window_length:1)
a = data$Soc28_Bad[complete.cases(data$Soc28_Bad),]
a = rollmeanr(a$Soc28_Bad,6,fill=NA)
a = a$Soc28_Bad[complete.cases(a$Soc28_Bad),]
#b = ROC(a$Soc28_Bad,24,type='discrete')
data$Soc28_Bad = NULL
b = a$Soc28_Bad[complete.cases(a$Soc28_Bad),]
c = roll_scale(b$Soc28_Bad[complete.cases(b$Soc28_Bad),],width=window_length,min_obs = 12)
plot(a)
plot(c)
data = merge(data,c,join='left',fill='NA')
```
...
window_length = 60
exp_decay <- 0.95 ^ (window_length:1)
a = data$Soc25_Better[complete.cases(data$Soc25_Better),]
b = rollmeanr(a$Soc25_Better,24,fill=NA)
b = b$Soc25_Better[complete.cases(b$Soc25_Better),]
#b = ROC(a$Soc25_Better,24,type='discrete')
data$Soc25_Better = NULL
b = b$Soc25_Better[complete.cases(b$Soc25_Better),]
c = roll_scale(b$Soc25_Better[complete.cases(b$Soc25_Better),],width=window_length,min_obs = 12)
plot(a)
plot(c)
data = merge(data,c,join='left',fill='NA')
```
...
window_length = 60
exp_decay <- 0.95 ^ (window_length:1)
a = data$Soc28_Relative[complete.cases(data$Soc28_Relative),]
b = rollmeanr(a$Soc28_Relative,24,fill=NA)
b = b$Soc28_Relative[complete.cases(b$Soc28_Relative),]
#b = ROC(a$Soc28_Relative,24,type='discrete')
data$Soc28_Relative = NULL
b = b$Soc28_Relative[complete.cases(b$Soc28_Relative),]
c = roll_scale(b$Soc28_Relative[complete.cases(b$Soc28_Relative),],width=window_length,min_obs = 12)
plot(a)
plot(c)
data = merge(data,c,join='left',fill='NA')
```
...
#window_length = 240
#exp_decay <- 0.95 ^ (window_length:1)
a = data$T10YFF[complete.cases(data$T10YFF),]
b = rollmeanr(a$T10YFF,12,fill=NA)
#c = roll_scale(b$T10YFF[complete.cases(b$T10YF),],width=window_length,min_obs = 12)
#data$T10YFF = NULL
plot(a)
plot(b)
plot(c)
...
complete_data = merge(complete_data,GSPC_12M_Ret,join='left',fill='NA')
complete_data = complete_data[complete.cases(complete_data),]


#Scales data by the complete data, which is inappropriate.
...
```{r}
L <- nrow(complete_data)-108
train.index = seq(1,L,by=1)
test.index = seq((L+1),nrow(complete_data),by=1)

preProcessParameters = preProcess(coredata(complete_data[train.index,1:(ncol(complete_data))]), method = c('center','scale'))
complete_stand_data = as.xts(predict(preProcessParameters,coredata(complete_data)),order.by=index(complete_data))
```


```{r, echo=FALSE}
preProcessParameters = preProcess(coredata(complete_data[train.index,1:(ncol(complete_data)-1)]), method = c('center','scale','pca'))
...
ggplot(resid_lrpca_test,aes(x=fitted,y=actual,colour=residual)) + geom_point() + scale_colour_gradientn(colours=rainbow(4)) + ggtitle(label = 'Test Residual Plot') + theme_dark()
```
...
```{r}
lr = lm(GSPC.Close ~ LADCB + CAPE + NYX_MD
        , complete_stand_data[train.index])
summary(lr)
...
plot(complete_stand_data$T10YFF)
```
...
```{r}
class_data = complete_stand_data
class_data$GSPC.Close = cut(class_data$GSPC.Close,c(-10,-1,10))
logr = glm(GSPC.Close ~ LADCB, class_data[train.index],family = 'gaussian')
summary(logr)
train_pred_logr = predict(logr,class_data[train.index,1:(ncol(complete_stand_data)-1)])
test_pred_logr = predict(logr,class_data[test.index,1:(ncol(complete_stand_data)-1)])
accuracy(as.vector(train_pred_logr),class_data[train.index,ncol(complete_stand_data)])
accuracy(as.vector(test_pred_logr),class_data[test.index,ncol(complete_stand_data)])
plot(logr)

scatter.smooth(logr$fit,
               residuals(logr, type = "deviance"),
               lpars = list(col = "red"),
...
model.full = lm(GSPC.Close ~ ., data = complete_stand_data[train.index]) #The model with ALL variables.
...
x_stand = sparse.model.matrix(GSPC.Close ~ LADCB, data = complete_stand_data)
...
```{r}
## Create model and assess accuracy on train/test set.
rf = randomForest(GSPC.Close ~ LADCB + T10YFF + CAPE + NYX_MD, complete_data[train.index])
varImpPlot(rf)
rf
...
```{r}
## Create model and assess accuracy on train/test set.
svr = svm(GSPC.Close ~ LADCB + CAPE + T10YFF + NYX_MD,complete_data[train.index],epsilon=.1,type='eps-regression',cross=5)
svr
...
```{r}
quant_reg = rq(GSPC.Close ~ LADCB + CAPE + T10YFF,tau=.53,complete_stand_data[train.index,])
...
```{r}
## Create model and assess accuracy on train/test set.
rf = randomForest(GSPC.Close ~ LADCB + CAPE + NYX_MD + T10YFF, complete_data[train.index])
varImpPlot(rf)
rf
train_pred_rf = predict(rf, complete_data[train.index,1:(ncol(complete_data)-1)])
test_pred_rf = predict(rf, complete_data[test.index,1:(ncol(complete_data)-1)])
...
```{r}
ct = ctree(GSPC.Close ~ UNRATE,complete_data[train.index], controls = ctree_control(maxsurrogate = 0, mtry = 0))
ct
plot(ct)
...
## Collect and visualize residuals from the train and test sets for analysis.
resid_ct = data.frame(complete_data[train.index,ncol(complete_data)],train_pred_ct,(complete_data[train.index,ncol(complete_data)] - train_pred_ct),row.names = index(complete_data[train.index]))
colnames(resid_ct) = c('actual','fitted','residual')
ggplot(resid_ct,aes(x=fitted,y=actual,colour=residual)) + geom_point() + scale_colour_gradientn(colours=rainbow(4)) + ggtitle(label = 'Train Residual Plot') + theme_dark()

resid_ct_test = data.frame(complete_data[test.index,ncol(complete_data)],test_pred_ct,(complete_data[test.index,ncol(complete_data)]-test_pred_ct),row.names = index(complete_data[test.index]))
colnames(resid_ct_test) = c('actual','fitted','residual')
ggplot(resid_ct_test,aes(x=fitted,y=actual,colour=residual)) + geom_point() + geom_jitter(width = .003,height = .003) + scale_colour_gradientn(colours=rainbow(4)) + ggtitle(label = 'Test Residual Plot') + theme_dark() + xlab('Fitted Standardized Return')
#Look at the 6- and 9- month rate of change for LADBC.  Both interesting.
```


```{r}
ct = rpart(GSPC.Close ~ .,complete_stand_data[train.index])
ct
plot(ct)

train_pred_ct = predict(ct, complete_stand_data[train.index,1:(ncol(complete_stand_data)-1)])
test_pred_ct = predict(ct, complete_stand_data[test.index,1:(ncol(complete_stand_data)-1)])
accuracy(as.vector(train_pred_ct), complete_stand_data[train.index,ncol(complete_stand_data)])
accuracy(as.vector(test_pred_ct), complete_stand_data[test.index,ncol(complete_stand_data)])

## Collect and visualize residuals from the train and test sets for analysis.
resid_ct = data.frame(complete_stand_data[train.index,ncol(complete_stand_data)],train_pred_ct,(complete_stand_data[train.index,ncol(complete_stand_data)] - train_pred_ct),row.names = index(complete_stand_data[train.index]))
colnames(resid_ct) = c('actual','fitted','residual')
ggplot(resid_ct,aes(x=fitted,y=actual,colour=residual)) + geom_point() + scale_colour_gradientn(colours=rainbow(4)) + ggtitle(label = 'Train Residual Plot') + theme_dark()

resid_ct_test = data.frame(complete_stand_data[test.index,ncol(complete_stand_data)],test_pred_ct,(complete_stand_data[test.index,ncol(complete_stand_data)]-test_pred_ct),row.names = index(complete_stand_data[test.index]))
colnames(resid_ct_test) = c('actual','fitted','residual')
ggplot(resid_ct_test,aes(x=fitted,y=actual,colour=residual)) + geom_point() + scale_colour_gradientn(colours=rainbow(4)) + ggtitle(label = 'Test Residual Plot') + theme_dark()
...
df = data.frame(complete_stand_data[,1:(ncol(complete_stand_data)-1)])
knn_clust = kmeans(df, centers =5, algorithm = 'MacQueen')
fviz_cluster(knn_clust, data = df, ellipse.type = "norm",
             ellipse.level = 0.8,ggtheme= theme_economist_white(),main = 'KNN Clustering')
...
L <- length(z)- 112  # shrink the length to avoid row index error
x <- z[1:L]
fit2<-auto.arima(x)
fc <- forecast(fit2, h = 6)  # the output would have error if the number of row > 1e4
plot(fc)
accuracy(fc, z[(L+1):(L+6)])
...
sqrt(mean(residuals.nnetar**2,na.rm=T))  # 0.007213
accuracy(nn_model)
fc_nn <- forecast(nn_model, h=6)
accuracy(fc_nn, z[(L+1):(L+6), 1])
plot(fc_nn)
plot(z[1:(L+6),1])
sd(z)
```
...
NNAR(5,1,4)[12] II
```{r}
plot(GSPC_12M_Ret,plot.type = "functions", plotlegend = TRUE)
