...
btc_max_rr_1 = xts(btc_max_rr_1, order.by = as.Date(usd$date))

trade_info = cbind(usd$trades, intra_day_fluc(usd$high,usd$low), btc_max_rr_1)
colnames(trade_info) = c('btc_usd_trades','btc_usd_in_day_fluc','btc_max_rr_1')
trade_info = xts(trade_info, order.by = as.Date(usd$date))
