...
driver.get("https://www.decks.de/decks-sess/workfloor/lists/list.php?wo=hon&nowstyle=zz")
...
while True:
    try:
        ...
        for row in rows:
            release_info = {}
            # >>>>>>>> All these if else follow the same pattern, so it is better to wrap them up in a function. <<<<<<<
