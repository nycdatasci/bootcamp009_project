...
train = read.csv(paste0(path,'train.csv'), row.names = 'id')
dim(train) # 30471 x 292
#macro = read.csv(paste0(path,'macro.csv')) # 2484 x 100

table(sapply(train,function(x) class(x)))
#factor integer numeric 
#16     185      91 
names(train[,sapply(train,function(x) class(x)=='factor')])
names(train[,sapply(train,function(x) class(x)=='integer')])
names(train[,sapply(train,function(x) class(x)=='numeric')])
