...
## Take five day moving average.
for (i in seq(ncol(price),6)) {
  price$p_sqf[i] = median(price$p_sqf[i-5:i])
  }  
...
reduced_mo_data = complete_mo_data
macro_regression = lm(p_sqf ~.,reduced_mo_data[,2:21])
summary(macro_regression)
plot(macro_regression)
```
