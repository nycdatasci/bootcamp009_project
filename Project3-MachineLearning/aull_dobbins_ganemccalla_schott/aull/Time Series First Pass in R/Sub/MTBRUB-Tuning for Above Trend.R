...
# Log transform (Leaving it turned off right now while doing EDA & initial modelling).
...
macro$lending_spread = runmean(macro$mortgage_rate,60)

plot(macro$lending_spread)
...
macro$lending_spread = Delt(macro$lending_spread, k = 140)
...
ggplot(macro,aes(x=timestamp,y=price_square_meter)) +
  geom_point(aes(color=brent_rub)) + geom_smooth(method='lm') +
  theme_excel() + scale_color_gradient2()

a = lm(price_square_meter ~ brent_rub + mortgage_rate + lending_spread + rent_price_1room_eco, macro)
summary(a)
BIC(a)

ggplot(macro,aes(x=timestamp,y=lending_spread)) +
  geom_point(aes(color=lending_spread)) + geom_smooth(method='lm') +
  theme_excel() + scale_color_gradient2()
