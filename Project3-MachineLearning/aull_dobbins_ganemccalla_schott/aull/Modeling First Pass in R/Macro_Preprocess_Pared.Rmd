...
train <- read_csv("./train.csv", 
    col_types = cols(build_year = col_integer(), 
        max_floor = col_integer(), timestamp = col_date(format = "%Y-%m-%d")))
colSums(is.na(train))
...
price$logprice = log(price$price_doc)
ggplot(data = price,aes(x=logprice)) + geom_histogram()
...
```{r}
pared_linear_regression = lm(price_doc ~ full_sq, complete_stand_data[,2:ncol(complete_stand_data)])
